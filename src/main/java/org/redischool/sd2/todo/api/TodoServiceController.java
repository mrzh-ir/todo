    ...
    private List<ItemDto> currentItems() {
        ...
        for (Item item : items) {
            ...
            if (item instanceof RecurringTask) {
                itemDto = ItemDto.recurringTaskWithLabel(item.getLabel());
            } else if (item instanceof ShoppingItem) {
                itemDto = ItemDto.shoppingItemWithLabel(item.getLabel());
            } else if (item instanceof OneTimeTask) {
                if (((OneTimeTask) item).getDeadline() != null) {
                    itemDto = ItemDto.oneTimeTaskWithLabelAndDeadline(item.getLabel(), ((OneTimeTask) item).getDeadline().toString());
                } else itemDto = ItemDto.oneTimeTaskWithLabel(item.getLabel());
            }
